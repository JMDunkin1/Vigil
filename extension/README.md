...
The extension checks each tab against `http://127.0.0.1:8787` on this Mac. It uses the same profiles, strict sessions, schedules, App Locks, Time Limits, and Open Limits as Sentinel. It also sends active-tab pulses so browser time limits can work even when AppleScript URL access is unavailable.
