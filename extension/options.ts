...
testButton.addEventListener("click", async () => {
  ...
  try {
    const response = await fetch(sentinelUrl("/api/state", localServer), { headers: { Accept: "application/json" } });
    const signature = response.headers.get("x-sentinel-app") || "";
    if (!response.ok || !signature.startsWith("tech.caseline.sentinel;")) {
      setDiagnostic(serverCheck, `No Sentinel response at ${localServer}.`);
  ...
  try {
    const response = await fetch(sentinelUrl("/api/extension/check", localServer), {
      method: "POST",
...
async function readPairing(localServer: string, extensionToken: string): Promise<PairingResponse | null> {
  try {
    const response = await fetch(sentinelUrl("/api/extension/pairing", localServer), {
...
function pairingFailureText(pairing: PairingResponse | null, detail: string, extensionToken: string): string {
  ...
  if (extensionToken && trust?.tokenConfigured && trust?.tokenSupplied) {
    return `Token not accepted. Check SENTINEL_EXTENSION_TOKEN. ${detail}`;
  }
  ...
  return `Not paired. Add this extension ID/origin to Sentinel, or enter the shared token. ${detail}`;
}

function defaultPairingEnv(): string {
  return [
    `SENTINEL_EXTENSION_ID=${chrome.runtime.id}`,
    `SENTINEL_EXTENSION_ORIGIN=${extensionOrigin}`
  ].join("\n");
}
