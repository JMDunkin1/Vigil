...
const SENTINEL_REQUEST_TIMEOUT_MS = 2500;
...
const RULE_SYNC_ALARM = "sentinel-rule-sync";
const RULE_EXPIRY_ALARM = "sentinel-rule-expiry";
...
async function performCheckUrl(
  ...
  try {
    response = await fetchSentinel("/api/extension/check", {
      method: "POST",
...
async function rememberPulseFlags(value: unknown): Promise<void> {
  ...
  await storageSet({ sentinelPulseFlags: cachedPulseFlags });
}

async function loadPulseFlags(): Promise<void> {
  const stored = await storageGet({ sentinelPulseFlags: {} });
  if (!isRecord(stored.sentinelPulseFlags)) return;
  const flags = stored.sentinelPulseFlags;
...
async function initializeSiteBlocking(): Promise<void> {
  await Promise.all([loadSentinelConnection(), pulseFlagsReady]);
...
function localServerAllowRule(): chrome.declarativeNetRequest.Rule {
  const origin = new URL(sentinelConnection.localServer).origin.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return {
...
async function fetchSentinel(path: string, options: RequestInit = {}): Promise<Response> {
  ...
  const timeout = setTimeout(() => controller.abort(), SENTINEL_REQUEST_TIMEOUT_MS);
  try {
    return await fetch(sentinelUrl(path, connection.localServer), {
      ...options,
      headers,
