...
export function createFocusSoundController({ $, post }) {
  const focusAudio = {
    context: null,
    gain: null,
    nodes: [],
  ...
  async function sync({ enabled, preset, volume, active }) {
  ...
  async function prime() {
    ...
    focusAudio.context ||= new AudioContext();
    if (focusAudio.context.state === "suspended") {
  ...
  function setVolume(value) {
    ...
    if (!gain) return;
    gain.gain.setTargetAtTime(volumeToGain(value), focusAudio.context.currentTime, 0.04);
  }
