...
interface SentinelJournalWindow extends Window {
  sentinelJournal?: {
    promptTouchId(): Promise<unknown>;
  };
}

interface SentinelAppearanceBridge {
  getIconTheme(): Promise<unknown>;
  setIconTheme(theme: string): Promise<unknown>;
}

interface SentinelAppearanceWindow extends Window {
  sentinelAppearance?: SentinelAppearanceBridge;
}
...
function bindIconThemeSettings(): void {
  const bridge = (window as SentinelAppearanceWindow).sentinelAppearance;
  const status = $("#iconThemeStatus");
  if (!bridge) {
    status.textContent = "Available in the Sentinel Mac app";
...
async function loadIconTheme(bridge: SentinelAppearanceBridge, status: HTMLElement): Promise<void> {
  try {
...
async function saveIconTheme(bridge: SentinelAppearanceBridge, theme: string, status: HTMLElement): Promise<void> {
  try {
...
async function unlockJournal(method: string, password: string): Promise<void> {
  ...
  try {
    ...
    if (method === "biometric") {
      const bridge = (window as SentinelJournalWindow).sentinelJournal;
      if (!bridge?.promptTouchId) throw new Error("Touch ID is available in the Sentinel Mac app.");
...
function renderJournalGate(data: DashboardData): void {
  ...
  const bridgeAvailable = Boolean((window as SentinelJournalWindow).sentinelJournal?.promptTouchId);
