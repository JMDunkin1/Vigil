...
final class SocialWebViewStore: NSObject, ObservableObject {
    ...
    private func resolveMedia(id: String, token: String, verdict: ContentSafetyVerdict, service: SocialService) {
        ...
        webViews[service]?.evaluateJavaScript("window.__sentinelResolveMedia?.(\(encodedID), \(encodedToken), '\(verdict.rawValue)');")
    }
    ...
    private func handlePageText(_ body: [String: Any], service: SocialService) {
        ...
        Task { [weak self] in
            ...
            self.webViews[service]?.evaluateJavaScript("window.__sentinelResolvePageText?.(\(encodedRevision), '\(verdict.rawValue)');")
        }
    }
