...
- `tech.caseline.sentinel.instagram` builds as **Instagram**.
- `tech.caseline.sentinel.youtube` builds as **YouTube**.
- `tech.caseline.sentinel.snapchat` builds as **Snapchat**.
...
The app code hard-codes only Sentinel's permanent YouTube Shorts and Snapchat Spotlight/Stories blocks. Configurable Instagram and other feature toggles remain the MDM web-filter policy's responsibility; the iPhone apps do not yet receive live `FocusedSocialSettings` from the desktop agent.
...
xcodebuild -allowProvisioningUpdates \
  ...
  -derivedDataPath /tmp/Sentinel-Instagram-device \
  PRODUCT_BUNDLE_IDENTIFIER=tech.caseline.sentinel.instagram \
  PRODUCT_NAME=Instagram \
  SOCIAL_APP_NAME=Instagram \
  SOCIAL_ICON_NAME=instagram.png \
  SENTINEL_SERVICE=instagram \
  SOCIAL_URL_SCHEME=sentinel-instagram \
  DEVELOPMENT_TEAM=3RY7A22U4L \
  CODE_SIGN_STYLE=Automatic build
```

Do not install the app or either ManageEngine profile until Sentinel has a current, complete iPhone layout checkpoint. The static launcher profile and dynamic enforcement profile are intentionally separate so Level changes cannot expire and recreate the launcher icons.
