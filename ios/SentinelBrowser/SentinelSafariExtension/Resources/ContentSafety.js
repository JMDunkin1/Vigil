(() => {
  "use strict";
  if (globalThis.__sentinelContentSafetyInstalled) return;
  globalThis.__sentinelContentSafetyInstalled = true;

  const nativeApplication = "tech.caseline.sentinel.browser";
  const isWebKitBrowser = Boolean(globalThis.webkit?.messageHandlers?.sentinelContentSafety);
  ...
  style.textContent = `
    ...
    *, *::before, *::after {
    ...
    [data-sentinel-media-verdict="sensitive"] { filter: blur(48px) !important; visibility: hidden !important; }
    html[data-sentinel-page-verdict="sensitive"] body,
    html[data-sentinel-page-verdict="unknown"] body { visibility: hidden !important; }
  `;
  ...
  const sendNative = payload => {
    if (isWebKitBrowser) {
      globalThis.webkit.messageHandlers.sentinelContentSafety.postMessage({ ...payload, frameID });
  ...
  globalThis.__sentinelResolveMedia = (id, token, verdict) => {
    ...
    element.dataset.sentinelMediaVerdict = verdict;
  };
  ...
  const submitMedia = element => {
    ...
    sendNative(payload).then(response => {
      if (!isWebKitBrowser) globalThis.__sentinelResolveMedia(id, token, response?.verdict || "unknown");
    }).catch(() => globalThis.__sentinelResolveMedia(id, token, "unknown"));
  };
