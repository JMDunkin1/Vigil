...
enum SharedFilterStore {
    static let appGroup = "group.tech.caseline.sentinel.browser"
    static let rulesKey = "SentinelBrowser.filterRules.v1"
...
struct NavigationFilter: Sendable {
    ...
    func decide(_ url: URL) -> FilterDecision {
        guard blocklistIntegrityValid else {
            return .block(reason: "Sentinel's content filter failed its integrity check.")
        }
        ...
        if rules.blockedHosts.contains(where: { host == $0 || host.hasSuffix(".\($0)") }) {
            return .block(reason: "This website is blocked by Sentinel.")
        }
        ...
        if rules.blockedURLFragments.contains(where: absolute.contains) {
            return .block(reason: "This page is blocked by Sentinel.")
        }
        if isBlockedSearch(url) {
            return .block(reason: "That search is blocked by Sentinel.")
        }
