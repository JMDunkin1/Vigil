...
final class BrowserStore: NSObject, ObservableObject {
    ...
    @Published private(set) var title = "Sentinel Browser"
    ...
    private func observeWebView() {
        observations = [
            webView.observe(\.title, options: [.new]) { [weak self] view, _ in
                Task { @MainActor in self?.title = view.title ?? "Sentinel Browser" }
            },
    ...
    private func installContentRules() {
        ...
        WKContentRuleListStore.default().compileContentRuleList(forIdentifier: "SentinelBrowser-\(rules.revision)", encodedContentRuleList: json) { [weak self] list, _ in
