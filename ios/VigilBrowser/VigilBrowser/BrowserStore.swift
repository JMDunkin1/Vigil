...
final class BrowserStore: NSObject, ObservableObject {
    ...
    init(
        ...
        configuration.userContentController.add(bridge, name: "vigilContentSafety")
    ...
    private func classifyMedia(_ body: [String: Any]) {
        ...
        Task { [weak self] in
            ...
            self.webView.evaluateJavaScript("window.__vigilResolveMedia?.(\(encodedID), \(encodedToken), '\(verdict.rawValue)');")
        }
    }
    ...
    private func classifyText(_ body: [String: Any]) {
        ...
        Task { [weak self] in
            ...
            self.webView.evaluateJavaScript("window.__vigilResolvePageText?.(\(encodedRevision), '\(verdict.rawValue)');")
