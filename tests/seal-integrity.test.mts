...
assert.equal(resolveDefaultDataDir(join("/tmp", "sentinel", "dist.nosync", "runtime")), join("/tmp", "sentinel", "data"));
assert.equal(
...

{
  ...
  try {
    ...
    await writeFile(join(dir, "src", "server 2.js"), "console.log('conflict');\n");
    await writeFile(join(dir, "scripts", "tool.mjs"), "console.log('tool');\n");
    await writeFile(join(dir, "scripts", "run-tests.mjs"), "console.log('tests');\n");
...

{
  ...
  try {
    ...
    const safariHistoryText = `${JSON.stringify(safariHistoryChange, null, 2)}\n`;
    assert.equal((await verifyStateTextSeal(safariHistoryText, { keyPath, sealPath })).status, "mismatch");

    const grayscaleHashChange = structuredClone(state);
    grayscaleHashChange.deviceControls.ios.mdm.lastGrayscaleHash = "forged-grayscale-hash";
    const grayscaleHashText = `${JSON.stringify(grayscaleHashChange, null, 2)}\n`;
    assert.equal((await verifyStateTextSeal(grayscaleHashText, { keyPath, sealPath })).status, "mismatch");
