...
try {
  ...
  assert.match(String(enrollmentBody.error), /Self-hosted Vigil MDM enrollment is not ready/);
  ...
  state.deviceControls.ios.mdm.enrollmentSecret = "root-enrollment-secret";
  state.deviceControls.ios.mdm.devices = [{ id: "device-record-1", udid: "iphone-raw-udid", status: "enrolled" }];
  const usageTokenResponse = mockResponse();
  const usageTokenHandled = await handleDeviceApiRoute(
    mockRequest("GET", "/api/devices/ios/mdm/device-usage-token?device=device-record-1", {}),
    usageTokenResponse,
    new URL("http://127.0.0.1:8787/api/devices/ios/mdm/device-usage-token?device=device-record-1"),
    {
      state,
      usage: {},
      recordIosMdmPolicyQueue: () => ({ queued: false })
  ...
  const usageTokenBody = JSON.parse(usageTokenResponse.bodyText) as Record<string, unknown>;
  assert.equal(usageTokenHandled, true);
  assert.equal(usageTokenResponse.statusCodeValue, 200);
  assert.equal(usageTokenBody.deviceId, "iphone-raw-udid");
  assert.match(String(usageTokenBody.token), /^[A-Za-z0-9_-]{43}$/);
  assert.notEqual(usageTokenBody.token, state.deviceControls.ios.mdm.enrollmentSecret);
  ...
  const usageHandled = await handleDeviceApiRoute(
    mockRequest("POST", "/api/devices/usage", {
      ...
      totalSeconds: 40 * 60,
      apps: {
        "com.burbn.instagram": 20 * 60,
        "com.google.ios.youtube": 20 * 60
      },
      sites: {
        "instagram.com": 20 * 60,
        "youtube.com": 20 * 60
