...
const dataDir = await mkdtemp(join(tmpdir(), "sentinel-persistence-integrity-"));
...
try {
  ...
  const weakenedMarker = JSON.parse(sealedStateText) as SentinelState;
  ...
  const unsealedText = JSON.parse(await readFile(store.STATE_PATH, "utf8")) as SentinelState;
