...
assert.equal(publicHostGuard({
  path: "/api/state",
  headers: { host: "sentinel.example.test" },
  remoteAddress: "127.0.0.1"
}).ok, false);
assert.equal(publicHostGuard({
  path: "/api/state",
...
assert.deepEqual(deviceUsageSyncAuthorization({
  headers: {
    host: "sentinel.example.test",
    "x-sentinel-device-id": "phone-udid",
    "x-sentinel-device-token": "device-secret"
  },
  url: new URL("https://sentinel.example.test/api/devices/usage"),
  deviceTokens: { "phone-udid": "device-secret" }
...
const previousHostedExtensionEnv = {
  auth: process.env.SENTINEL_AUTH_ENABLED,
  origin: process.env.SENTINEL_EXTENSION_ORIGIN,
  token: process.env.SENTINEL_EXTENSION_TOKEN
};
try {
  process.env.SENTINEL_AUTH_ENABLED = "1";
  process.env.SENTINEL_EXTENSION_ORIGIN = "chrome-extension://trusted-extension";
  process.env.SENTINEL_EXTENSION_TOKEN = "hosted-extension-secret";
  assert.equal(extensionRequestGuard({
    method: "POST",
    headers: {
      host: "sentinel.example.test",
  ...
  restoreEnv("SENTINEL_AUTH_ENABLED", previousHostedExtensionEnv.auth);
  restoreEnv("SENTINEL_EXTENSION_ORIGIN", previousHostedExtensionEnv.origin);
  restoreEnv("SENTINEL_EXTENSION_TOKEN", previousHostedExtensionEnv.token);
}
