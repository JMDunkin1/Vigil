...
try {
  ...
  await writeFile(join(liveSourceRoot, "package.json"), JSON.stringify({ name: "vigil" }));
  ...
  assert.deepEqual(resolvePublicAssets("/packaged/runtime", {
    VIGIL_LIVE_SOURCE: "1",
    VIGIL_SOURCE_ROOT: liveSourceRoot
  }), {
...
assert.equal(diagnosticExportFilename(new Date("2026-06-04T12:34:56.789Z")), "vigil-diagnostic-2026-06-04T12-34-56Z.json");
...
diagnosticState.deviceControls.ios.mdm.publicBaseUrl = "https://vigil.example.test";
...
assert.equal(recordValue(exportedState.distanceKey, "diagnostic distance key").keyFilePath, "/Volumes/Vigil/key.txt");
