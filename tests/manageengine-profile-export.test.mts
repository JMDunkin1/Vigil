...
const projectRoot = basename(root) === "runtime" && ["dist", "dist.nosync"].includes(basename(runtimeParent))
  ...
  : root;
const dataDir = await mkdtemp(join(tmpdir(), "sentinel-manageengine-export-"));
...
try {
  ...
  const profilePath = join(dataDir, "sentinel-manageengine-policy.mobileconfig");
  const summaryPath = join(dataDir, "sentinel-manageengine-policy.summary.json");
  const launcherProfilePath = join(dataDir, "sentinel-social-launchers.mobileconfig");
  const launcherSummaryPath = join(dataDir, "sentinel-social-launchers.summary.json");
  ...
  const savedState = JSON.parse(await readFile(join(dataDir, "state.json"), "utf8")) as SentinelState;
  ...
  await writeFile(launcherObservationPath, JSON.stringify({
    observedAt: "2026-07-10T12:00:00.000Z",
    installedProfileIdentifier: "com.local-screen-time.ios-social-launchers",
  ...
  const noSuffixProfilePath = join(dataDir, "sentinel-manageengine-policy-no-suffix");
...

{
  const artifact = {
    ...
    profileIdentifier: "com.local-screen-time.ios-lock",
...
{
  const overlapDir = await mkdtemp(join(tmpdir(), "sentinel-manageengine-overlap-"));
  try {
    const outPath = join(overlapDir, "sentinel-manageengine-policy.mobileconfig");
    const summaryPath = join(overlapDir, "sentinel-manageengine-policy.summary.json");
    ...
    const launcherPath = join(overlapDir, "sentinel-social-launchers.mobileconfig");
...
function runExporter(args: string[], sentinelDataDir: string): Promise<{ code: number; stdout: string; stderr: string }> {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [join(root, "scripts", "export-manageengine-ios-profile.mjs"), ...args], {
      cwd: root,
      env: {
        ...process.env,
        SENTINEL_DATA_DIR: sentinelDataDir
      },
