...
assert.equal(browserPageNeedsProtection('Safari','https://example.com/',16001),true,'changing URL does not renew the loading allowance');
assert.equal(recordBrowserFilterHealth('Safari',url,'old',16001),false);
assert.equal(recordBrowserFilterHealth('Google Chrome',url,BROWSER_FILTER_REVISION,16001),true);
assert.equal(browserPageNeedsProtection('Safari',url,16002),true,'Chrome cannot vouch for Safari');
assert.equal(recordBrowserFilterHealth('Safari',url,BROWSER_FILTER_REVISION,16003),true);
assert.equal(browserPageNeedsProtection('Safari',url,16004),false);
assert.equal(browserPageNeedsProtection('Safari','https://other.example/',17000),false);
assert.equal(browserPageNeedsProtection('Safari','https://other.example/',23001),true,'another website cannot borrow the lease');
assert.equal(browserPageNeedsProtection('Safari',url,25000),true,'disabled/stale extensions fail closed');
