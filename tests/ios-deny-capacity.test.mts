...
for (const profileId of [NORMAL_PROFILE_ID, SOFT_BLOCK_PROFILE_ID, BRICK_MODE_PROFILE_ID]) {
  ...
  for (const host of ["kinklets.com", "protonvpn.com", "croxyproxy.com", "browser.lol", "invidious.f5.si"]) {
...
for (const candidate of candidates) {
  assert.ok(customDenied.some(prefix => candidate.startsWith(prefix)), `${candidate} must remain covered`);
}
