...
const root = await mkdtemp(join(await realpath(tmpdir()), "vigil-distance-key-"));
try {
  const state = defaultState();
  const keyPath = join(root, "removable", "vigil.key");
  ...
  assert.throws(() => updateDistanceKeySettings(state, {
    keyFilePath: join(linkedDirectory, "vigil.key"),
    writeKeyFile: true
  }), /symbolic link/i);
  await assert.rejects(readFile(join(actualDirectory, "vigil.key")), /ENOENT/u);
} finally {
