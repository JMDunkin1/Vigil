...

{
  ...
  assert.equal(matchAdultBlocklistHost(state, "media.pornhub.com")?.sourceId, "sentinel-explicit");
...
{
  const rotationDir = await mkdtemp(join(tmpdir(), "sentinel-adult-snapshot-"));
  try {
