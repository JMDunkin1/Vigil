...
const preloadBridge = exposed.get("sentinelAppUpdate") as Bridge | undefined;
...
assert.equal(invocations[0]?.channel, "sentinel:app-update-status");
...
assert.equal(invocations[1]?.channel, "sentinel:app-update-start");
assert.equal(invocations[1]?.args.length, 0);
const appearanceBridge = exposed.get("sentinelAppearance") as AppearanceBridge | undefined;
...
assert.equal(invocations[2]?.channel, "sentinel:icon-theme-get");
assert.equal(invocations[3]?.channel, "sentinel:icon-theme-set");
assert.deepEqual(invocations[3]?.args, ["sacred-heart"]);
assert.match(mainSource, /ipcMain\.handle\("sentinel:app-update-status", handleAppUpdateStatus\)/u);
...
const installRoot = await mkdtemp(join(tmpdir(), "sentinel-app-update-install-"));
try {
  const stageRoot = join(installRoot, "dist", "update-mac.noindex");
  const builtApp = join(stageRoot, "mac-arm64", "Sentinel.app");
  const installedApp = join(installRoot, "dist", "mac.noindex", "mac-arm64", "Sentinel.app");
  ...
  assert.equal(await readFile(join(`${installedApp}.sentinel-previous`, "version.txt"), "utf8"), "old");
...
const rendererBridge = {
  ...
  async start() {
    startCalls += 1;
    return { ok: true, supported: true, phase: "starting", message: "Sentinel will reopen" };
  }
};

Object.defineProperty(globalThis, "window", {
  configurable: true,
  value: { sentinelAppUpdate: rendererBridge }
});
