...
const dataRoot = await mkdtemp(join(tmpdir(), "vigil-mdm-token-migration-"));
process.env.VIGIL_DATA_DIR = join(dataRoot, "state");
...
assert.equal(DATA_DIR, process.env.VIGIL_DATA_DIR, "the migration server test must stay bound to its temporary data directory");
...
state.deviceControls.ios.mdm = {
  ...
  topic: "com.apple.mgmt.vigil-migration-test",
