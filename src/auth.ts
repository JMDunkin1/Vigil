...
export const BOOTSTRAP_TOKEN_HEADER = "x-sentinel-bootstrap-token";
...
export function hostedSignupsEnabled(): boolean {
  return hostedAccountsEnabled() && truthy(process.env.SENTINEL_SIGNUPS_ENABLED);
}
...
function bootstrapTokenMatches(request: IncomingMessage): boolean {
  const expected = String(process.env.SENTINEL_BOOTSTRAP_TOKEN || "");
  const supplied = requestHeader(request, BOOTSTRAP_TOKEN_HEADER);
  if (!expected || !supplied) return false;
  const comparisonKey = "sentinel-bootstrap-token-comparison";
