...
import type { KeyholderState, SentinelState, UnknownRecord } from "./types.js";
...
export function keyholderSummary(state: SentinelState) {
...
export function updateKeyholderSettings(state: SentinelState, body: UnknownRecord = {}, now = new Date()) {
  const current = state.keyholder;
...
export function assertKeyholderPasscode(state: SentinelState, passcode: unknown): void {
  const current = state.keyholder;
