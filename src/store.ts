...
async function recoverMalformedState(raw: Buffer, error: unknown, now = new Date()): Promise<SentinelState> {
  ...
  if (!evidence.complete) {
    ...
    console.error("Sentinel could not preserve all corrupt-state evidence; state writes are suppressed to preserve the originals:", evidence.error);
  }
  ...
  const detail = evidence.complete
    ...
    : `State file is invalid. Sentinel could not preserve all evidence, so the original files remain in place and state writes are disabled. ${parseDetail}`;
  ...
  if (evidence.complete) {
    try {
      ...
      console.error("Sentinel preserved corrupt state evidence but could not persist the fail-closed recovery state:", saveError);
    }
  }
  return recovered;
}

export async function loadUsage(state: SentinelState): Promise<UsageState> {
  await ensurePrivateDirectory(DATA_DIR);
...
function markUsageSealRequired(state: SentinelState, now = new Date()): void {
...
async function recoverUsageConservatively(state: SentinelState, reason: string, now = new Date()): Promise<UsageState> {
...
function conservativeUsageState(state: SentinelState, now = new Date()): UsageState {
...
function normalizeLimitRules(existingRules: SentinelState["limitRules"], builtinRules: SentinelState["limitRules"]): SentinelState["limitRules"] {
...
function cloneLimitRule(rule: SentinelState["limitRules"][number]): SentinelState["limitRules"][number] {
  return {
    ...rule,
    apps: [...rule.apps],
    sites: [...rule.sites],
