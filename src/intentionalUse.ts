...
function plannerSummary(state: VigilState, now: Date) {
...
function recoverySummary(state: VigilState, now: Date) {
  ensureIntentionalUse(state);
  const day = dateKey(now);
  const week = weekKey(now);
...
function sosPlan(state: VigilState, { intent, trigger, replacement }: { intent: string; trigger: string; replacement: string }): string[] {
