...
import type { ActivePolicy, DeviceTarget, DeviceTargetInput, LimitBlock, LimitProgress, LimitRule, VigilState, UsageSample, UsageState } from "./types.js";
...
export function activeLimitBlocks(state: VigilState, now = new Date(), options: { device?: DeviceTargetInput } = {}): LimitBlock[] {
  cleanupExpiredLimitBlocks(state, now);
...
function limitRuleContextMatches(state: VigilState, rule: LimitRule, sample: UsageSample, now: Date): boolean {
...
function activeContextMatchesProfile(state: VigilState, profileId: string, now: Date, device: DeviceTarget): boolean {
...
function limitRuleOverridden(state: VigilState, ruleId: string, now: Date): boolean {
