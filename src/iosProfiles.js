...
export function iosProfileSummary(state, now = new Date()) {
  ...
  return {
    ...
    removalHardened: Boolean(settings.hardenRemoval && settings.removalPassword),
    ...
    profile: {
      ...
      fileName: "local-screen-time-iphone-lock.mobileconfig",
...
export function buildIosConfigurationProfile(state, now = new Date()) {
  ...
  if (settings.hardenRemoval && settings.removalPassword) {
  ...
  const profile = {
    ...
    PayloadDescription: "Locks distracting iPhone apps and websites using supervised Apple device-management restrictions generated by Local Screen Time.",
    PayloadDisplayName: "Local Screen Time iPhone Lock",
    PayloadIdentifier: IOS_PROFILE_IDENTIFIER,
    PayloadOrganization: APP_NAME,
    PayloadRemovalDisallowed: Boolean(settings.hardenRemoval),
