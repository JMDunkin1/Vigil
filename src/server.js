...
async function handleMdm(request, response, url) {
  ...
  if (method === "GET" && path === "/mdm/enroll.mobileconfig") {
    ...
    await saveState(state);
    sendDownload(response, 200, profile, "local-screen-time-iphone-mdm.mobileconfig", "application/x-apple-aspen-config");
    return;
  }
  ...
  if (method === "GET" && path === "/mdm/policy.mobileconfig") {
    ...
    await saveState(state);
    sendDownload(response, 200, profile, "local-screen-time-iphone-lock.mobileconfig", "application/x-apple-aspen-config");
    return;
  }
...
<body>
  <main>
    ...
    <p class="meta">Locked until ${until || "the session ends"}. Dashboard: <a href="http://127.0.0.1:${PORT}">Local Screen Time</a></p>
  </main>
  <script>
