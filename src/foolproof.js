...
export function foolproofSummary(state, context = {}, now = new Date()) {
...
export function assertFoolproofReadyForStrict(state, context = {}, now = new Date()) {
...
export function foolproofBlockers(state, context = {}, now = new Date()) {
...
export function extensionVersionReady(state) {
...
export function extensionRecentlySeen(state, now = new Date()) {
  const lastSeen = Date.parse(state.extension?.lastSeenAt || "");
  return Number.isFinite(lastSeen) && now.getTime() - lastSeen < EXTENSION_FRESH_MS;
}

export function extensionDynamicRulesReady(state, now = new Date()) {
  ...
  const syncedAt = Date.parse(dynamicRules.syncedAt || "");
