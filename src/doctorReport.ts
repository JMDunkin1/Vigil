...
export function doctorRows(state: VigilState, context: DoctorContext = {}, now = new Date()): DoctorRow[] {
  ...
  return [
    ...
    row("content-filter", "Content filter", true, "Apple's built-in Safari web filter stays on, with Vigil deny URLs and precise browser companion checks layered on top."),
