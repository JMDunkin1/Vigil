...
export function buildHostsBlock(state: SentinelState, now = new Date()): string {
...
export function managedBlockDomains(state: SentinelState, now = new Date()): string[] {
...
export async function hostsStatus(state: SentinelState | null = null, now = new Date()) {
  try {
...
export async function stateSealStatus(state: SentinelState | null = null) {
  try {
    ...
    if (!parsed) {
      try {
        parsed = JSON.parse(text) as SentinelState;
      } catch {
        return {
          ok: false,
...
export async function loadStateForScript(): Promise<SentinelState> {
  return JSON.parse(await readFile(STATE_PATH, "utf8")) as SentinelState;
}
