...
const LEGACY_APP_NAME = "Local Screen Time";
const CURRENT_APP_NAME = "Sentinel";
...
export function applySealVerificationToState(state: SentinelState, verification: SealVerification, now = new Date()): void {
...
export function markStateSealed(state: SentinelState, sealedAt = new Date().toISOString()): void {
  state.integrity.stateSeal ||= {};
  const seal = state.integrity.stateSeal;
...
export function stateSealSummary(state: Partial<SentinelState> | null | undefined, liveVerification: SealVerification | null = null) {
  const seal = state?.integrity?.stateSeal || {};
...
function trustedProtectedStateMigrationVariants(snapshot: ProtectedSnapshot): Array<{ snapshot: ProtectedSnapshot; detail: string }> {
  ...
  if (legacyBranding) {
    variants.push({
      ...
      detail: "State file changed only by the trusted Local Screen Time to Sentinel branding migration; the seal can be refreshed without entering lockdown."
    });
  }
