...
export function extensionTrustSummary(headers: HeaderBag = {}, transport: RequestTransportContext = {}): ExtensionTrustSummary {
  ...
  return {
    ...
    suggestedOriginEnv: normalized ? `SENTINEL_EXTENSION_ORIGIN=${normalized}` : null,
    suggestedIdEnv: extensionId ? `SENTINEL_EXTENSION_ID=${extensionId}` : null,
    suggestedTokenEnv: "SENTINEL_EXTENSION_TOKEN=<shared-token>"
  };
}
...
function configuredPublicHosts(): Set<string> {
  return new Set(csvEnv("SENTINEL_PUBLIC_HOSTS").map(normalizedHost).filter(Boolean));
}
