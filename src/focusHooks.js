...
export function focusShortcutSummary(state) {
...
export async function reconcileFocusShortcut(state, policy, now = new Date()) {
  state.focusShortcut ||= {};
  const settings = state.settings || {};
  const status = state.focusShortcut;
...
export function focusShortcutDetail(summary) {
