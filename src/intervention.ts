import type { SentinelState, StateEvent } from "./types.js";
...
type AttemptWithTarget = StateEvent & { target: AttemptTarget };
type CountedAttemptTarget = AttemptTarget & { count: number };

export function interventionSummary(state: SentinelState, now = new Date()) {
...
export function emergencyDelaySeconds(state: SentinelState, now = new Date()): number {
  return interventionSummary(state, now).emergencyDelaySeconds;
}

export function recentBlockAttempts(state: SentinelState, now = new Date(), windowMinutes = 10): AttemptWithTarget[] {
...
function interventionSettings(state: SentinelState): InterventionSettings {
...
function baseEmergencyDelaySeconds(state: SentinelState): number {
  return clamp(state.settings?.emergencyDelaySeconds, 1, 100000, 45);
}

function topAttemptTargets(attempts: AttemptWithTarget[]): CountedAttemptTarget[] {
  const counts = new Map<string, CountedAttemptTarget>();
...
function eventTarget(event: StateEvent): AttemptTarget {
...
function interventionMessage(attempts: number, settings: InterventionSettings, extraDelaySeconds: number): string {
