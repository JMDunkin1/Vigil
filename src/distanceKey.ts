...
import type { DistanceKeyState, SentinelState, UnknownRecord } from "./types.js";
...
export function distanceKeySummary(state: SentinelState) {
...
export function updateDistanceKeySettings(state: SentinelState, body: UnknownRecord = {}, now = new Date()) {
  const current = state.distanceKey;
...
export function assertDistanceKey(state: SentinelState, token: unknown, now = new Date()): void {
  const current = state.distanceKey;
