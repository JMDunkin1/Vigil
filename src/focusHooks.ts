...
export function focusShortcutSummary(state: SentinelState) {
...
export async function reconcileFocusShortcut(state: SentinelState, policy: ActivePolicy | null, now = new Date()) {
  const settings = state.settings;
  const status = state.focusShortcut;
