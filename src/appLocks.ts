...
import type { ActivePolicy, AppLockRequest, AppLockRule, AppLockUnlock, SentinelState, UnknownRecord, UsageSample } from "./types.js";

type TargetLists = { apps: string[]; sites: string[] };
type UnlockInput = UnknownRecord & { challengeText?: string };

export function activeAppLockPolicy(state: SentinelState, sample: UsageSample, now = new Date()): ActivePolicy | null {
...
export function requestAppLockUnlock(state: SentinelState, lockId: string, reason = "", now = new Date()): AppLockRequest {
  cleanupAppLockState(state, now);
...
export function confirmAppLockUnlock(state: SentinelState, requestId: string, input: UnlockInput | Date = {}, now = new Date()): AppLockUnlock {
...
function findLock(state: SentinelState, lockId: string): AppLockRule | undefined {
  return (state.appLocks || []).find((lock) => lock.id === lockId);
}

function hasActiveUnlock(state: SentinelState, lockId: string, now: Date): boolean {
  return Boolean(activeUnlockFor(state, lockId, now));
}

function activeUnlockFor(state: SentinelState, lockId: string, now: Date): AppLockUnlock | null {
  return (state.appLockUnlocks || []).find((unlock) => unlock.lockId === lockId && new Date(unlock.until) > now) || null;
}

function pendingRequestFor(state: SentinelState, lockId: string, now: Date): AppLockRequest | null {
...
function remainingUnlocks(state: SentinelState, lock: AppLockRule, now: Date): number {
  return Math.max(0, (lock.unlocksAllowed || 0) - usedToday(state, lock.id, now));
}

function usedToday(state: SentinelState, lockId: string, now: Date): number {
  return state.appLockLedger?.[dateKey(now)]?.[lockId] || 0;
}

function bumpLedger(state: SentinelState, lockId: string, now: Date): void {
...
function sampleMatchesLock(state: SentinelState, lock: AppLockRule, sample: UsageSample): boolean {
...
function cleanupAppLockState(state: SentinelState, now: Date): void {
