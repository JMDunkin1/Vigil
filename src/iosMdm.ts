...
export function iosMdmDoctor(state: SentinelState, now = new Date()) {
  ...
  return {
    ...
    warnings: [
      ...
      doctorItem(
        ...
        "apple",
        "Sentinel can check local shape, but cannot prove Apple MDM credentials until an iPhone enrolls and APNs accepts a push.",
...
function iosMdmSetupDiagnostics(mdm: Partial<MdmSettings> = {}): IosMdmDoctorItem[] {
  ...
  if (!mdm.identityCertificateUuid) {
    blockers.push(doctorItem(
      ...
      "Set SENTINEL_MDM_IDENTITY_UUID to the PayloadUUID for the PKCS#12 identity payload.",
      ["SENTINEL_MDM_IDENTITY_UUID"]
    ));
  }
  ...
  if (!mdm.identityCertificatePayloadBase64) {
    blockers.push(doctorItem(
      ...
      "A placeholder string is not enough; iOS needs a real identity payload or a real SCEP design that Sentinel does not yet implement.",
      "Set SENTINEL_MDM_IDENTITY_P12 to a real identity .p12 file.",
      ["SENTINEL_MDM_IDENTITY_P12", "SENTINEL_MDM_IDENTITY_P12_PASSWORD"]
    ));
    ...
    blockers.push(doctorItem(
      ...
      "Sentinel only stores the base64 PKCS#12 bytes; fake text or malformed base64 will produce an enrollment profile that cannot work.",
      "Export a real identity certificate as PKCS#12 and pass it with SENTINEL_MDM_IDENTITY_P12.",
      ["SENTINEL_MDM_IDENTITY_P12", "SENTINEL_MDM_IDENTITY_P12_PASSWORD"]
    ));
  }
  return blockers;
  ...
  if (!mdm.pushCertificatePayloadBase64) {
    ...
    if (!pushStatus.ok) {
      blockers.push(doctorItem(
        ...
        "apns",
        pushStatus.message,
        "Sentinel can only push through APNs with a real MDM push certificate exported as PKCS#12.",
