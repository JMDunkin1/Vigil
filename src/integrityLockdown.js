...
export function integrityLockdownActive(state) {
  return Boolean(activeIntegrityAlarm(state));
}

export function integrityLockdownPolicy(state, now = new Date()) {
...
export function clearIntegrityTamper(state, now = new Date()) {
...
export function detectRuntimeGap(state, now = new Date()) {
  const runtime = ensureRuntime(state);
...
export function detectClockTamper(state, sample = {}, now = new Date()) {
  const runtime = ensureRuntime(state);
...
export function recordRuntimeHeartbeat(state, now = new Date()) {
  const runtime = ensureRuntime(state);
...
function activeIntegrityAlarm(state) {
...
function strictScheduleOverlap(schedule, startMs, endMs, currentWifi = "") {
...
function scheduleWindowForStartDay(schedule, day) {
...
function scheduleEnvironmentMatches(schedule, currentWifi) {
