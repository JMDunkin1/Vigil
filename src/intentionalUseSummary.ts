...
import type { IntentionalBehavior, IntentionalJournalEntry, IntentionalRecoveryCheckIn, VigilState } from "./types.js";

export function behaviorSummary(state: VigilState, behavior: IntentionalBehavior, currentWeekKey: string) {
...
export function plannerSummary(state: VigilState, now: Date) {
...
export function journalEntriesForWeek(state: VigilState, currentWeekKey: string): IntentionalJournalEntry[] {
  return (state.intentionalUse.journalEntries || []).filter((entry) => weekKey(new Date(entry.entryDate || entry.createdAt)) === currentWeekKey);
}

export function reflectionStreakDays(state: VigilState, now: Date): number {
...
export function recoverySummary(state: VigilState, now: Date) {
...
export function sosPlan(state: VigilState, { intent, trigger, replacement }: { intent: string; trigger: string; replacement: string }): string[] {
