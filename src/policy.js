...
const SITE_ALIAS_LOOKUP = new Map();
...
const APP_ALIAS_LOOKUP = new Map();
...
export function scheduleWindow(schedule, now = new Date()) {
...
export function isStrictUnsupportedBrowser(appName) {
  return appMatchesAppTargets(appName, STRICT_UNSUPPORTED_BROWSERS);
}

export function isStrictEmbeddedBrowserApp(appName) {
  return appMatchesAppTargets(appName, STRICT_EMBEDDED_BROWSER_APPS);
}

export function shouldBlockSite(profile, hostname) {
...
export function shouldBlockUrl(profile, value) {
  return Boolean(matchBlockedUrlPattern(profile, value));
}

export function matchStrictBrowserControlUrl(state, policy, value) {
...
export function matchBlockedUrlPattern(profile, value) {
  if (!profile) return null;
...
export function expandSiteTargets(values) {
  const expanded = [];
...
export function expandAppTargets(values) {
  const expanded = [];
...
export function appMatchesAppTargets(appName, targets) {
  const app = normalizeAppName(appName);
  if (!app) return false;
  return expandAppTargets(targets).includes(app);
}

export function isProcessSweepExemptApp(appName) {
...
export function hostMatchesSiteTargets(hostname, targets) {
  const host = normalizeHost(hostname);
  if (!host) return false;
...
export function normalizeUrlPattern(value) {
  const input = String(value || "").trim().toLowerCase();
  if (!input) return "";
  try {
...
function strictBypassAppsForPolicy(state, policy) {
...
function policyUsesSiteBlocking(policy) {
...
function browserControlArea(url) {
...
function urlPatternCandidates(url) {
...
function isScheduleOverridden(state, scheduleId, endsAt, now) {
...
function makeWindow(now, startMinutes, endMinutes, addEndDays, startOffsetDays = 0) {
...
function phase(kind, label, blocking, round, rounds, startsAt, endsAt) {
  return {
    kind,
    label,
    blocking,
