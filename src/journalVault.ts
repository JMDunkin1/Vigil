...
import type { JournalVaultState, SentinelState, UnknownRecord } from "./types.js";

const PASSWORD_MIN_LENGTH = 6;
const PASSWORD_MAX_LENGTH = 256;
const SESSION_TOKEN_BYTES = 32;
const HASH_BYTES = 32;
const TOUCH_ID_SECRET_HEADER = "x-sentinel-touch-id-secret";
const DEFAULT_DATA_DIR = process.env.SENTINEL_DATA_DIR || resolveDefaultDataDir(dirname(dirname(fileURLToPath(import.meta.url))));
...
export function journalVaultSummary(state: SentinelState) {
...
export function setJournalVaultPassword(state: SentinelState, body: UnknownRecord = {}, now = new Date()) {
  const vault = normalizeJournalVaultState(state.intentionalUse.journalVault || {});
...
export function unlockJournalVaultWithPassword(state: SentinelState, body: UnknownRecord = {}, now = new Date()) {
  const vault = normalizeJournalVaultState(state.intentionalUse.journalVault || {});
  if (!journalVaultConfigured(vault)) {
...
export function requireJournalVaultSession(
  ...
  cleanupSessions(now);
  const token = headerValue(headers["x-sentinel-journal-token"]);
