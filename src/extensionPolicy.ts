...
export function extensionRuleSnapshot(state: VigilState, now = new Date()) {
...
function publicPauseOverlay(state: VigilState, pause: IntentionalPause | null | undefined, now: Date) {
  if (!pause) return null;
...
function browserNoiseBlockingEnabled(state: VigilState): boolean {
  return integrityLockdownActive(state) || state.settings?.browserNoiseBlockingEnabled !== false;
}

function focusedSocialCleanupEnabled(state: VigilState, now: Date): boolean {
...
function focusedSocialCleanupSettingsForState(state: VigilState, now: Date): FocusedSocialSettings {
...
function blockingPolicyFor(state: VigilState, usage: UsageState, sample: UsageSample, now: Date): ActivePolicy | null {
  const sessionPolicy = activePolicy(state, now);
...
function adultBlocklistPolicyFor(state: VigilState, hostname: string | undefined, now: Date): ActivePolicy | null {
...
function matchContentFilterForActivePolicy(state: VigilState, url: URL, now: Date) {
  const policy = activePolicy(state, now);
...
function safeBackUrl(state: VigilState, value: unknown, currentUrl: URL, now: Date): string {
...
function isVigilUrl(url: URL): boolean {
