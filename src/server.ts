...
function manageEnginePolicyOutputPaths(): { outPath: string; summaryPath: string } {
  const dir = process.env.SENTINEL_MANAGEENGINE_DIR || defaultManageEngineExportDir();
  return {
    outPath: join(dir, "sentinel-manageengine-policy.mobileconfig"),
    summaryPath: join(dir, "sentinel-manageengine-policy.summary.json")
  };
}

function defaultManageEngineExportDir(): string {
  const explicitRoot = process.env.SENTINEL_REPO_ROOT || "";
