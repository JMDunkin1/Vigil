...
function profileDeleteBlocker(state: VigilState, id: string): string {
...
function upsertGrayscaleSchedule(state: VigilState, body: UnknownRecord): GrayscaleSchedule {
  state.grayscale ||= {
...
function upsertLimitRule(state: VigilState, body: UnknownRecord): LimitRule {
  const id = stringValue(body.id, randomUUID());
...
function upsertAppLock(state: VigilState, body: UnknownRecord): AppLockRule {
  const id = stringValue(body.id, randomUUID());
