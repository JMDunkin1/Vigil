...
export function createLocalScriptRunner({ root, launchAgentStatus, processObject = process }: LocalScriptRunnerOptions) {
  ...
  function localScriptEnvOverrides(): Record<string, string> {
    ...
    if (processObject.env.SENTINEL_DATA_DIR) overrides.SENTINEL_DATA_DIR = processObject.env.SENTINEL_DATA_DIR;
    if (processObject.env.SENTINEL_PORT) overrides.SENTINEL_PORT = processObject.env.SENTINEL_PORT;
    if (processObject.env.SCREEN_TIME_PORT) overrides.SCREEN_TIME_PORT = processObject.env.SCREEN_TIME_PORT;
    return overrides;
  }
