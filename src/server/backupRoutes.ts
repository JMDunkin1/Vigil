...
export function buildBackupExport({ state, usage, activePort, startedAt, now = new Date() }: BackupExportInput) {
  ...
  return {
    ...
    metadata: {
      ...
      restore: {
        supported: false,
        reason: "Restore/import is not exposed because Sentinel state is protected by integrity seals and should be reintroduced only through a verified migration flow."
      }
    },
...
export function backupExportFilename(now = new Date()): string {
  ...
  return `sentinel-backup-${stamp}.json`;
}
