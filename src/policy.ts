...
import type { ActivePolicy, DeviceTarget, DeviceTargetInput, LockLevel, PolicyPhase, PolicyPhaseKind, Profile, Schedule, VigilState, Session } from "./types.js";
...
export function activeProfile(state: VigilState): Profile {
  return state.profiles.find((profile) => profile.id === state.settings.activeProfileId) || state.profiles[0];
}

export function baselineProfile(state: VigilState): Profile {
...
export function profileById(state: VigilState, id: string): Profile {
...
export function baselinePolicy(state: VigilState, now = new Date(), options: PolicyDeviceOptions = {}): ActivePolicy | null {
...
export function activePolicy(state: VigilState, now = new Date(), options: PolicyDeviceOptions = {}): ActivePolicy | null {
  const device = normalizeDeviceTarget(options.device);
...
export function clearSessionsById(state: VigilState, sessionId: unknown): DeviceTarget[] {
...
export function isStrictBypassAppForPolicy(state: VigilState, policy: ActivePolicy | null | undefined, appName: unknown): boolean {
...
export function matchStrictBrowserControlUrl(state: VigilState, policy: ActivePolicy | null | undefined, value: unknown): {
  area: string;
  label: string;
  url: string;
} | null {
...
function shouldApplyStrictBypassProtection(state: VigilState, policy: ActivePolicy | null | undefined): boolean {
...
function strictBypassAppsForPolicy(state: VigilState, policy: ActivePolicy | null | undefined): string[] {
...
function cleanupExpired(state: VigilState, now: Date): void {
...
function isScheduleOverridden(state: VigilState, scheduleId: string, endsAt: string, now: Date): boolean {
