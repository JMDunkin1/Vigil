...
This is the hosted-MDM path for a solo Vigil install. ManageEngine owns Apple APNs enrollment and device wakeups; Vigil exports the supervised iPhone policy as a custom configuration profile.
...
- Vigil's self-hosted remote MDM doctor is still expected to report APNs certificate blockers. That is fine for this path because ManageEngine, not Vigil, is the MDM server.
