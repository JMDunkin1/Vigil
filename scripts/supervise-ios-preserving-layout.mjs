...
if (!options.confirm) {
  throw new Error([
    ...
    "This flow creates a full local backup, supervises the iPhone, restores that backup to recover Home Screen layout, then applies Vigil.",
...
if (!options.skipApplyProfile) await applyVigilProfile(udid, supervisorKeybagPath, checkpointRoot);
...
async function applyVigilProfile(udid, supervisorKeybagPath, checkpointRoot = "") {
  const args = [
