...
if (!options.confirm) {
  throw new Error([
    ...
    "This slow path protects user data, Home Screen layout, folders, Apple ID setup state, and settings before Sentinel touches supervision.",
    "It creates or verifies a full local backup, applies a tiny setup-state restore to clear Apple's activated-device cloud-configuration gate, supervises without erasing, restores the full backup to recover layout/settings, re-supervises if the restore clears supervision, pairs supervised, then applies Sentinel.",
...
console.log([
  `Sentinel layout-preserving iPhone setup starting for ${udid}.`,
...
console.log("Running no-erase supervision with the persistent Sentinel supervisor identity.");
...
if (!isSupervisedCloud(restoredCloud)) {
  console.log("Layout restore cleared supervision; re-running no-erase supervision before applying Sentinel.");
  await superviseDevice(udid, options.organization, supervisorKeybagPath);
...
if (!options.skipApplyProfile) await applySentinelProfile(udid, supervisorKeybagPath, checkpointRoot, options.password);
...
async function resolveExistingCheckpointRoot(inputPath, udid) {
  ...
  if (basename(backup.path) !== udid) {
    throw new Error([
      ...
      "Sentinel will not restore a backup unless the backup folder identity matches the connected device."
    ].join("\n"));
  }
  console.log([
...
async function createPreSupervisionRestorePayload(checkpointRoot, udid, password = "") {
  ...
  if (JSON.stringify(payload.files) !== JSON.stringify(expectedFiles) || JSON.stringify(payload.entries) !== JSON.stringify(expectedEntries)) {
    throw new Error([
      ...
      "Sentinel stopped before rewriting setup state or supervising the phone."
    ].join("\n"));
  }
  console.log([
...
async function waitForCloudConfigurationCleared(udid, phase, timeoutMs) {
  ...
  throw new Error([
    ...
    "Sentinel stopped before no-erase supervision because Apple's activated-device cloud-configuration gate is still present.",
...
async function superviseDevice(udid, organization, supervisorKeybagPath) {
  ...
  if (/cloud configuration is already present/i.test(output)) {
    throw new Error([
      ...
      "Sentinel stopped before backup restore/profile install.",
