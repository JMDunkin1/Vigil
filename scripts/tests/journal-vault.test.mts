...
assert.equal(requireJournalVaultSession(state, {
  "x-sentinel-journal-token": passwordSession.token
...
assert.equal(revokeJournalVaultSession({ "x-sentinel-journal-token": secondSession.token }), true);
assert.throws(() => requireJournalVaultSession(state, {
  "x-sentinel-journal-token": secondSession.token
}, now), /Unlock the journal/);

const previousSecret = process.env.SENTINEL_TOUCH_ID_SECRET;
process.env.SENTINEL_TOUCH_ID_SECRET = "test-touch-id-secret";
const touchSession = unlockJournalVaultWithTouchId(state, {
  "x-sentinel-touch-id-secret": "test-touch-id-secret"
}, now);
assert.equal(touchSession.method, "touch-id");
assert.throws(() => unlockJournalVaultWithTouchId(state, {
  "x-sentinel-touch-id-secret": "wrong"
}, now), /not accepted/);
if (previousSecret === undefined) delete process.env.SENTINEL_TOUCH_ID_SECRET;
else process.env.SENTINEL_TOUCH_ID_SECRET = previousSecret;
