...
assert.equal(commitmentLockError({ kind: "panic" }), "Panic lockout cannot be ended early.");
...
const runner = createLocalScriptRunner({
  root: "/Applications/Sentinel.app/Contents/Resources/app.asar",
...
assert.equal(
  ...
  "/Applications/Sentinel.app/Contents/Resources/app.asar.unpacked/scripts/apply-hosts.mjs"
);
...
assert.match(privilegedCommand, /app\.asar\.unpacked\/scripts\/apply-hosts\.mjs/);
