...
const dataRoot = await mkdtemp(join(tmpdir(), "sentinel-manageengine-auto-export-"));
const stateDir = join(dataRoot, "state");
const manageEngineDir = join(dataRoot, "manageengine");
process.env.SENTINEL_DATA_DIR = stateDir;
process.env.SENTINEL_MANAGEENGINE_DIR = manageEngineDir;
...
try {
  ...
  const profilePath = join(manageEngineDir, "sentinel-manageengine-policy.mobileconfig");
  const summaryPath = join(manageEngineDir, "sentinel-manageengine-policy.summary.json");
  const launcherProfilePath = join(manageEngineDir, "sentinel-social-launchers.mobileconfig");
  const launcherSummaryPath = join(manageEngineDir, "sentinel-social-launchers.summary.json");
  ...
  assert.equal(profile.PayloadIdentifier, "com.local-screen-time.ios-lock");
