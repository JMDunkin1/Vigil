...
export async function validateRestorableBackupPayload({
  ...
  try {
    ...
    throw new Error([
      ...
      "Sentinel fully traversed the backup manifest and will not use this recovery source for a device mutation unless every file entry has a valid hashed payload.",
      errorDetail(error)
    ].filter(Boolean).join("\n"), { cause: error });
  }
}
