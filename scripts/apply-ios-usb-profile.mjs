...
if (!isSupervisedCloud(cloud)) {
  throw new Error([
    ...
    "Sentinel uses the same supervised-device enforcement model as SHIFT, and iOS will not accept app or web restriction payloads until the phone is supervised.",
    "Sentinel will not attempt a partial-restore/no-erase supervision trick from this script because that can disturb Apple ID, setup, and Home Screen layout state; a checkpoint proves a recovery source exists, not that the supervision flow is safe.",
    "Use `npm run ios:supervise-preserve-layout -- --yes-supervise-and-restore` to create a verified layout checkpoint, supervise with a persistent Sentinel keybag, restore the checkpoint, then apply this profile."
  ].join("\n"));
}
...
if (!profilePath) {
  ...
  if (installedAfterRemove) {
    throw new Error(`Sentinel profile remove command completed, but ${IOS_PROFILE_IDENTIFIER} is still present in the device profile list.`);
  }
  console.log([
    `Sentinel iPhone profile removed over USB from ${udid}.`,
    `Profile: ${IOS_PROFILE_IDENTIFIER}`,
    "Reason: Sentinel has no active iPhone policy."
  ].join("\n"));
  process.exit(0);
}
...
if (!installed) {
  throw new Error(`Sentinel profile install command completed, but ${IOS_PROFILE_IDENTIFIER} was not found in the device profile list.`);
}

console.log([
  `Sentinel iPhone profile applied over USB to ${udid}.`,
  `Profile: ${IOS_PROFILE_IDENTIFIER}`,
...
async function verifyBackupDevice(backupRoot, udid) {
  ...
  if (ids.length && !ids.includes(udid)) {
    throw new Error([
      ...
      "Sentinel will not apply a USB profile until the recovery checkpoint belongs to this iPhone."
    ].join("\n"));
  }
  if (!ids.length && basename(backupRoot) !== udid) {
    throw new Error([
      ...
      "The backup metadata does not expose a device identifier, so Sentinel cannot prove this recovery source belongs to the connected iPhone.",
...
async function prepareIosServerState() {
  const state = await sentinelJson("/api/state");
  const ios = state?.state?.deviceControls?.ios;
  if (!ios || typeof ios !== "object" || Array.isArray(ios)) {
    throw new Error("Sentinel API /api/state did not include state.deviceControls.ios; cannot safely preserve iOS settings before applying over USB.");
  }
  ...
  return { state: await sentinelJson("/api/state"), active: true };
}
...
async function sentinelJson(path, options = {}) {
  const response = await fetch(`${SENTINEL_SERVER}${path}`, {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Sentinel API ${path} failed: HTTP ${response.status}${detail ? ` ${detail}` : ""}`);
  }
...
async function requireSupervisorKeybag(requestedPath = "") {
  ...
  if (!keybag?.isFile() || keybag.size <= 0) {
    throw new Error([
      ...
      "Pass it with --supervisor-keybag=/path/to/supervisor.keybag, set SENTINEL_SUPERVISOR_KEYBAG, or place it at data/sentinel-supervisor.keybag.",
      "Sentinel will not create a new keybag here because a new supervision identity cannot pair with a device supervised by Apple Configurator, SHIFT, or another existing identity."
    ].join("\n"));
  }
