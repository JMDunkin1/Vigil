...
const profileLabel = options.profile
  ...
  : "the active Vigil iPhone profile from the local server";
...
while (true) {
  ...
  if (result.code === 0) {
    writeOutput(result);
    console.log("Vigil USB profile install completed.");
    process.exit(0);
  }
  ...
  if (!waitingMessage) {
    writeOutput(result);
    console.error("Vigil USB profile install failed before a safe retry point.");
...
Options:
  --profile PATH              Install a specific .mobileconfig instead of the active Vigil profile.
