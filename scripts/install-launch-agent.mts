...
if (!home) throw new Error("HOME is required to install the Sentinel LaunchAgent.");
const homeDir = home;
const uid = process.getuid?.();
if (uid === undefined) throw new Error("process.getuid is required to install the Sentinel LaunchAgent.");
...
const installedRuntimeRoot = join(home, "Library", "Application Support", "Sentinel", "agent-runtime");
...
try {
  ...
  if (rollbackErrors.length) {
    throw new AggregateError([error, ...rollbackErrors], "Sentinel installation failed and could not be fully rolled back.");
  }
  throw error;
}
...
async function installRuntime(
  ...
  const nextPath = `${destination}.sentinel-next`;
  const previousPath = `${destination}.sentinel-previous`;
  const reinstallingFromInstalledRuntime = source === destination;
  if (await pathExists(previousPath)) {
    throw new Error(`A previous Sentinel runtime recovery copy still exists at ${previousPath}.`);
  }
  ...
  return {
    ...
    async rollback() {
      if (backedUp) {
        if (!(await pathExists(previousPath))) {
          throw new Error(`Sentinel runtime recovery copy is missing at ${previousPath}.`);
        }
        await rm(destination, { recursive: true, force: true });
        await rename(previousPath, destination);
      } else if (installed) {
...
async function resolveSourceRoot(): Promise<string> {
  ...
  if (existingPlist !== null) {
    const existingSourceRoot = plistStringForKey(existingPlist, "SentinelSourceRoot");
...
async function resolveLaunchAgentDataDir(): Promise<string> {
  if (process.env.SENTINEL_DATA_DIR) return process.env.SENTINEL_DATA_DIR;
  const existingPlist = await optionalRead(plistPath);
  if (existingPlist !== null) {
    ...
    if (existing.source === "working-directory") {
      ...
      if (existing.dataDir !== currentDataDir) {
        const [existingHasState, currentHasState] = await Promise.all([
          hasSentinelState(existing.dataDir),
          hasSentinelState(currentDataDir)
        ]);
        if (launchAgentDataRootsConflict(existing.dataDir, currentDataDir, existingHasState, currentHasState)) {
          throw new Error(
            `Sentinel found state in both the existing LaunchAgent data directory ${existing.dataDir} and the current runtime data directory ${currentDataDir}. Set SENTINEL_DATA_DIR explicitly before reinstalling the LaunchAgent.`
          );
        }
      }
  ...
  const applicationSupportData = join(homeDir, "Library", "Application Support", "Sentinel");
  const [repositoryHasState, applicationSupportHasState] = await Promise.all([
    hasSentinelState(repositoryData),
    hasSentinelState(applicationSupportData)
  ]);
  if (repositoryHasState && applicationSupportHasState) {
    throw new Error(
      `Sentinel found state in both ${repositoryData} and ${applicationSupportData}. Set SENTINEL_DATA_DIR explicitly before installing the LaunchAgent.`
    );
  }
  return repositoryHasState ? repositoryData : applicationSupportData;
}

async function hasSentinelState(directory: string): Promise<boolean> {
  for (const name of [
    "state.json",
    "usage.json",
    "accounts.json",
    ...
    try {
      ...
      // Try the next Sentinel state file.
    }
  }
  return false;
}
