...
import type { SentinelState, UnknownRecord } from "../src/types.js";
...
copyValue(body, "enabled", args.enable ?? env("SENTINEL_MDM_ENABLED"));
copyValue(body, "publicBaseUrl", args["public-base-url"] ?? env("SENTINEL_MDM_PUBLIC_BASE_URL"));
copyValue(body, "topic", args.topic ?? env("SENTINEL_MDM_TOPIC"));
copyValue(body, "identityCertificateUuid", args["identity-uuid"] ?? env("SENTINEL_MDM_IDENTITY_UUID"));
copyValue(body, "identityCertificatePassword", args["identity-password"] ?? env("SENTINEL_MDM_IDENTITY_P12_PASSWORD"));
copyValue(body, "pushCertificatePassword", args["push-password"] ?? env("SENTINEL_MDM_PUSH_P12_PASSWORD"));
copyValue(body, "useDevelopmentApns", args["development-apns"] ?? env("SENTINEL_MDM_DEVELOPMENT_APNS"));

const identityPath = args["identity-p12"] ?? env("SENTINEL_MDM_IDENTITY_P12");
const pushPath = args["push-p12"] ?? env("SENTINEL_MDM_PUSH_P12");
...
if (Object.keys(body).length === 0) {
  console.error([
    ...
    "Use --enable plus --public-base-url, --topic, --identity-uuid, --identity-p12, and --push-p12, or set the matching SENTINEL_MDM_* env vars.",
...
if (json) {
  ...
  console.log(`${dryRun ? "Dry run" : "Saved"} advanced self-hosted Sentinel MDM settings.`);
