...
const commonName = sanitizeSubjectValue(String(args["common-name"] || `Sentinel MDM ${identityUuid.slice(0, 8)}`)).slice(0, 64);
const passwordFromEnv = process.env.SENTINEL_MDM_IDENTITY_P12_PASSWORD;
...
await execOpenSsl([
  ...
  outPath,
  "-name",
  "Sentinel iPhone MDM Identity",
  "-passout",
  passout
]);
await chmod(outPath, 0o600).catch(() => {});

console.log("Created Sentinel local MDM identity PKCS#12.");
...
if (generatedPassword) {
  ...
  console.log("Password: read from SENTINEL_MDM_IDENTITY_P12_PASSWORD");
}
console.log("");
console.log("Configure with:");
console.log(`SENTINEL_MDM_IDENTITY_UUID=${identityUuid} \\`);
