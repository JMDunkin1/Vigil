...
async function main(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      log.write(`[${new Date().toISOString()}] Local Vigil could not start: ${errorMessage(error)}\n`);
    }
    if (exitCode === 0) return;
    log.write(`[${new Date().toISOString()}] Local Vigil exited with status ${exitCode}. Reopening ${options.appPath}.\n`);
    await reopenInstalledApp(options.appPath, log);
