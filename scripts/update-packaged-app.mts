...
try {
  ...
  if (!dirty) {
    await status("pulling", "Fast-forwarding Sentinel source");
    await run("git", ["merge", "--ff-only", "@{u}"], { allowFailure: true });
  } else {
    await status("rebuilding", "Rebuilding current checkout");
  }

  await status("building", "Building Sentinel runtime");
  await run("npm", ["run", "build"]);

  await status("packing", "Refreshing packaged Sentinel app");
  await refreshPackagedApp();

  if (options.restart) {
    await status("relaunching", "Reopening Sentinel");
    await run("/usr/bin/open", [options.appPath]);
  }
  await status("complete", "Sentinel update complete", { ok: true, finishedAt: new Date().toISOString() });
} catch (error) {
