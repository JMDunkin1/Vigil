...
if (options.confirm) {
  throw new Error([
    "Standalone Home Screen restore is disabled because a system/settings restore can clear iPhone supervision.",
    "Layout records must be included in the single escrow-backed pre-supervision restore documented in AGENTS.md.",
    "Run without --yes-restore-layout to prepare and validate a payload for inspection. No device mutation was started."
  ].join("\n"));
}
...
console.log("Inspection only; the iPhone was not changed. Standalone layout restore is disabled.");
