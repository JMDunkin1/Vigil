...
console.log([
  "Prepared Sentinel iPhone Home Screen layout restore payload.",
  `UDID: ${udid}`,
  `Backup: ${backupRoot}`,
  `Payload: ${payloadRoot}`,
...
console.log([
  "Sentinel restored the selected iPhone Home Screen layout payload.",
...
async function ensureRestorePairing(udid, supervisorKeybag = "") {
  ...
  if (!keybag?.isFile() || keybag.size <= 0) {
    throw new Error([
      ...
      "Pass --supervisor-keybag=/path/to/supervisor.keybag or place it at data/sentinel-supervisor.keybag."
    ].join("\n"));
  }
