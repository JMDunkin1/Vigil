...
const state = structuredClone(savedState) as VigilState;
...
async function persistRemovalPasswordForHardenedExport(savedState: VigilState, exportState: VigilState): Promise<boolean> {
...
function prepareManageEngineState(state: VigilState, parsed: Record<string, string | boolean>, windowMode: boolean): void {
...
function buildSummary(state: VigilState, windowMode: boolean, outputPath: string, stateSaved: boolean) {
  const ios = state.deviceControls.ios;
