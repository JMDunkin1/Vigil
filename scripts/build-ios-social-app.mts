...
const project = "ios/SentinelSocial/SentinelSocial.xcodeproj";
const valueOptions = new Set(["service", "configuration", "destination", "derived-data"]);
const services = {
  instagram: {
    bundleId: "tech.caseline.sentinel.instagram",
    name: "Instagram",
    icon: "instagram.png",
    scheme: "sentinel-instagram"
  },
  youtube: {
    bundleId: "tech.caseline.sentinel.youtube",
    name: "YouTube",
    icon: "youtube.png",
    scheme: "sentinel-youtube"
  },
  snapchat: {
    bundleId: "tech.caseline.sentinel.snapchat",
    name: "Snapchat",
    icon: "snapchat.png",
    scheme: "sentinel-snapchat"
  }
} as const;
...
export function buildArguments(argv: string[]): string[] {
  ...
  const args = [
    ...
    `SENTINEL_SERVICE=${options.service}`,
