...
5. The workflow publishes only the stapled DMG and `release-checksums.json`. This checksum sidecar records the marketing version, macOS build version, artifact byte length, and SHA-256 digest; it is not an automatic update feed.
6. Distribution-signed installs are updated by installing a complete published DMG. The source-checkout updater rejects them by design; it is not a production update mechanism.
